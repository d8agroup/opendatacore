from datauploader.models import DataUploadRecord, DataUploadProgress
...
class DataUploadController(object):
    ...
    def AllAvailableUploaders(cls):
        ...
        """
        import datauploader.lib as data_uploader_lib
        path = os.path.dirname(data_uploader_lib.__file__)
        data_uploader_directories = [d for d in os.listdir(path) if not re.search(r'\.', d)]
        data_uploaders = [getattr(my_import('datauploader.lib.%s.datauploader' % d), 'DataUploader')() for d in data_uploader_directories]
